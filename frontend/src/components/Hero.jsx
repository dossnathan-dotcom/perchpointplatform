...
import { HEADLINES, IMAGES, RENTALS } from '@/data/siteData';
  ...
  return <section id="top" className="relative overflow-hidden bg-obsidian pt-20 text-linen" aria-label="HawkVision Homes" data-testid="public-hero">
    ...
    <div className="relative mx-auto max-w-7xl px-5 pb-10 pt-14 sm:px-8 sm:pt-20">
      ...
      <form onSubmit={search} className="grid gap-3 border border-white/30 bg-linen p-4 text-obsidian md:grid-cols-[1fr_1fr_auto]" data-testid="hero-rental-search-form">
        {[['Location', 'location', ['All locations', ...new Set(RENTALS.map((u) => u.neighborhood))]], ['Space', 'use', ['All', 'Residential', 'Commercial', 'Mixed-use']]].map(([label,key,options]) => <label key={key} className="grid gap-2 px-2 text-xs text-stone-700">{label}<select className="min-h-10 w-full border-b border-stone-400 bg-transparent text-sm font-semibold text-obsidian" value={filters[key]} onChange={(e) => setFilters({...filters,[key]:e.target.value})} data-testid={`hero-search-${key}-select`}>{options.map((o) => <option key={o}>{o}</option>)}</select></label>)}
