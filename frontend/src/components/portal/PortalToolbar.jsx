...
import { DENSITIES, THEMES, applyDocumentTheme, storePreference } from '@/design-system/theme';
...
export const PortalToolbar = ({ role, onRoleChange, onMenu, onCommands, search, onSearch, context, onContext, onLogout }) => {
  ...
  return <header className="relative border-b border-white/20 bg-obsidian p-4 sm:px-7" data-testid="portal-toolbar">
    ...
    <div className="mt-3 flex flex-wrap items-center gap-3"><label className="grid min-w-0 flex-1 gap-1 text-xs text-linen/75">Organization / property context<select className="h-10 w-full max-w-full border border-white/40 bg-obsidian px-2 text-linen" value={narrow ? PORTFOLIO.properties[0].id : context} disabled={narrow} onChange={(e) => onContext(e.target.value)} data-testid="portal-context-select">{!narrow && <option value="all">HawkVision Homes · Demonstration portfolio</option>}{(narrow?PORTFOLIO.properties.slice(0,1):PORTFOLIO.properties).map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
      ...
      <label className="grid gap-1 text-xs text-linen/75">Theme<select className="h-10 border border-white/40 bg-obsidian px-2 text-linen" defaultValue="dark" data-testid="portal-theme-select" onChange={(event) => { storePreference("pp-theme", event.target.value, THEMES); applyDocumentTheme(window.location.pathname); }}>{THEMES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label className="grid gap-1 text-xs text-linen/75">Density<select className="h-10 border border-white/40 bg-obsidian px-2 text-linen" defaultValue="comfortable" data-testid="portal-density-select" onChange={(event) => { storePreference("pp-density", event.target.value, DENSITIES); applyDocumentTheme(window.location.pathname); }}>{DENSITIES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
    </div>
