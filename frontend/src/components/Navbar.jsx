  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-obsidian/90 text-linen shadow-2xl shadow-black/10 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8">
        <a href="#top" className="flex items-center gap-3" data-testid="navbar-brand-logo">
        ...
        <nav className="hidden items-center gap-8 lg:flex" data-testid="navbar-nav-links">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
