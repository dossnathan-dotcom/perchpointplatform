...
import { ArrowUpRight, BedDouble, Bath, Heart, Ruler } from "lucide-react";
...
export const Listings = ({ onSchedule, savedIds, onToggleSaved }) => {
            ...
            return (
              ...
              >
                <div className={`relative overflow-hidden ${index === 0 ? "h-[420px]" : "h-72"}`}>
                  ...
                  <button
                    aria-label={isSaved ? `Remove ${property.title} from saved homes` : `Save ${property.title}`}
                    aria-pressed={isSaved}
                    onClick={() => onToggleSaved(property.id)}
                    className={`absolute right-5 top-5 flex h-11 w-11 items-center justify-center rounded-full border backdrop-blur transition-colors ${isSaved ? "border-gold bg-gold text-obsidian" : "border-white/30 bg-obsidian/50 text-linen hover:border-gold hover:text-gold"}`}
                    data-testid={`property-card-${index + 1}-save-btn`}
                  >
                  ...
                  <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4 text-linen">
                    <div>
                      ...
                      <Link to={`/property/${property.id}`} className="mt-2 block font-heading text-2xl font-bold transition-colors hover:text-gold sm:text-3xl" data-testid={`property-card-${index + 1}-story-link`}>
                        {property.title}
                      </Link>
                    </div>
                ...
                <div className="p-6 sm:p-7">
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    <Button className="rounded-full bg-copper text-linen hover:bg-copperDark" onClick={() => onSchedule(property, "private_tour")} data-testid={`property-card-${index + 1}-tour-btn`}>
                    ...
                    >
                      <Link to={`/property/${property.id}`} data-testid={`property-card-${index + 1}-analyze-ai-btn`}>
                        View Full Story
                      </Link>
                    </Button>
                  </div>
