...
CI builds both images, validates Compose, and scans them with Trivy. Unfixed upstream findings are reported and do not by themselves fail the job. Findings with an available fix at high or critical severity fail the job. No image is pushed to a registry.
